...
useEffect(()=>{
    ...
    return(
        <div className="w-full h-20 md:h-25 flex justify-between items-center px-4 md:px-8">
            ...
            <form action="" className="flex items-center gap-2 px-4 bg-white w-[65%] md:w-[70%] h-[40%] md:h-[60%] rounded-md shadow-md" onSubmit={(e)=>e.preventDefault()}>
                <IoSearch className="w-5 h-5 text-blue-500"/>
                <input type="text" placeholder="Search Items...." className="w-full outline-none text-[12px] md:text-[16px]" onChange={(e)=>setInput(e.target.value)} value={input}/>
            </form>

            <div className="w-10 md:w-15 h-10 md:h-15 bg-white flex justify-center items-center rounded-md shadow-xl relative cursor-pointer" onClick={()=>
                setShowCart(true)
            }>
